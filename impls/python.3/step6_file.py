...
if __name__ == '__main__':
    ...
    repl_env['eval'] = core.make_eval(lambda ast: EVAL(ast, repl_env))
    repl_env['*ARGV*'] = t.MalList([t.MalString(a) for a in sys.argv[2:]])

    if len(sys.argv) > 1:
        code = 0
        try:
            filename = pr_str(t.MalString(sys.argv[1]), print_readably=True)
